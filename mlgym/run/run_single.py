...
class RunSingle:
    ...
    def main(self):
        ...
        env: MLGymEnv = gym.make(f"mlgym/{self.args.environment.task.id}", devices=agent_device).unwrapped  # type: ignore
        
        try:
            self.run(agent, env, agent_device, 0)
